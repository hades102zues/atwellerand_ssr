import React from "react";
import styles from "./Services.module.css";

interface ServiceCardInfo {
  iconUrl: any;
  serviceTitle: string;
  serviceDescription: string;
}

interface Props {
  showHeader: boolean;
}
const services: React.FC<Props> = (props) => {
  //will hold data to be injected into the services cards
  const servicesInputs: ServiceCardInfo[] = [
    {
      iconUrl:
        "data:image/png;base64,",
      serviceTitle: "Errands",
      serviceDescription:
        "Grocery Shopping, Package Deliveries and Event Planning. Let us take care of the simple day-to-day tasks.",
    },
    {
      iconUrl:
        "data:image/png;base64,",
      serviceTitle: "Property Management",
      serviceDescription:
        " Includes Tenant Screening,  Property Maintenance, Banking and Collection of Rent. Leave the hassle of management to us.",
    },
    {
      iconUrl:
        "data:image/png;base64,",
      serviceTitle: "Transportation",
      serviceDescription:
        " Chauffeuring, Airport Transfers, Island Tours and Personal Shuttles. We cover all of your transportation needs. ",
    },

    {
      iconUrl:
        "data:image/png;base64,",
      serviceTitle: "Rentals",
      serviceDescription: "Cars and Houses Rentals.  We have what you need.",
    },
  ];

  //generates the list of service cards
  const serviceCards: JSX.Element[] = servicesInputs.map(
    (config: ServiceCardInfo, index: number) => (
      <article className={styles.service} key={index}>
        <div className={styles.service_icon_box}>
          <img src={config.iconUrl} alt="service__icon" />
        </div>

        <h4 className={styles.service__title}>{config.serviceTitle.trim()}</h4>
        <p className={styles.service__description}>
          {config.serviceDescription.trim()}
        </p>
      </article>
    )
  );

  return (
    <section className={`${styles.sec_padding} ${styles.sec_grey}`}>
      <div className={styles.wrapper}>
        {props.showHeader ? (
          <h2 className={styles.section__title}>Services</h2>
        ) : null}
        <div className={styles.services_space}>
          <div className={styles.services_box}>{serviceCards}</div>
        </div>
      </div>
    </section>
  );
};

export default services;
